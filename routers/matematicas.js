...
routerMatematicas.get("/:tema/:nivel", (req, res) => {
  ...
  resultado.length > 0
    ...
    : res
        .status(404)
        .send(`No se encontraron cursos de matematicas para el tema: ${tema} y nivel: ${nivel}`);
});
...
// PUT: Reemplazo absoluto/completo de un curso por su ID
routerMatematicas.put("/:id", (req, res) => {
  const cursoActualizado = req.body;
  const id = req.params.id;
  
  // Busco en qué posición del array está el curso usando doble igual (==) porque el ID de la URL es un string
  const indice = matematicas.findIndex((curso) => curso.id == id);
  
  if (indice >= 0) {
    // Si existe, piso el objeto viejo por completo con el nuevo body
